...
async def get_stats(request: Request):
    ...
    cursor.execute("SELECT COUNT(*) as total FROM contacts WHERE status='replied'")
    ...
    cursor.execute("SELECT COUNT(*) as total FROM contacts WHERE created_at >= datetime('now', '-7 days')")
...
async def login(form: LoginForm):
    ...
    payload = {
        "sub": user["username"],
        "exp": (datetime.now() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)).timestamp(),
...
<div class="meta"> {c["email"]} |  {c["created_at"]} | 👁️ {c["views"] or 0} 次查看</div>
